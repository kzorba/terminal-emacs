...
COPY dotfiles/zsh/zshrc /tmp/.zshrc
# Copy emacsuser public key
COPY emacsuser.pub /tmp/emacsuser.pub

RUN chown -R ${USER_UID}:${USER_GID} /h/${USERNAME}/.doom.d && \
    chown ${USER_UID}:${USER_GID} /tmp/.zshrc && \
    chown ${USER_UID}:${USER_GID} /h/${USERNAME}/.gitconfig && \
    chown ${USER_UID}:${USER_GID} /tmp/emacsuser.pub
...
# libvterm, doom sync and env 
RUN ~/.emacs.d/bin/doom sync -U -b --force && \
    /bin/zsh -i -c "~/.emacs.d/bin/doom env" && \
...
RUN apt-get purge -y cmake libtool-bin && \
